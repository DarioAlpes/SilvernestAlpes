...
internal class EntradasAPIPruebas : PruebasUsandoServidorMock<EntradasAPI>()
{
    companion object
    {
    ...
    private val entidadNegocio = Entrada(1, 1, "Prueba", true, true, Precio(Decimal.DIEZ, 1), "código externo", 1)
    private val entidadDTO = EntradaDTO(entidadNegocio)

    override fun ManejadorDePeticiones.extraerApi(): EntradasAPI = apiDeEntradas

    override val mockParser: ParserRespuestasRetrofit = mockConDefaultAnswer(ParserRespuestasRetrofit::class.java).also { mock ->
        doAnswer {
            it.getArgument<() -> Response<EntradaDTO>>(0).invoke()
            RespuestaIndividual.Exitosa(entidadNegocio)
        }
            .`when`(mock)
            .haciaRespuestaIndividualDesdeDTO(cualquiera<() -> Response<EntradaDTO>>())
        ...
        doAnswer {
            ...
            .`when`(mock)
            .haciaRespuestaIndividualColeccionDesdeDTO(cualquiera<() -> Response<List<EntradaDTO>>>())
    }
    ...
    inner class Crear
    {
        ...
        fun invoca_el_metodo_de_parseo_de_respuesta_individual_dto()
        {
            ...
            verify(mockParser).haciaRespuestaIndividualDesdeDTO(cualquiera<() -> Response<EntradaDTO>>())
        ...
        fun invoca_url_correcta_usa_POST()
        {
            ...
            assertEquals("/clients/$ID_CLIENTE/entries", peticionRealizada.path)
    ...
    inner class Consultar
    {
        ...
        fun invoca_el_metodo_de_parseo_de_respuesta_individual_de_una_coleccion_dto()
        {
            ...
            verify(mockParser).haciaRespuestaIndividualColeccionDesdeDTO(cualquiera<() -> Response<List<EntradaDTO>>>())
            verifyNoMoreInteractions(mockParser)
        }
    ...
    inner class Actualizar
    {
        ...
        fun invoca_url_correcta_usa_PUT()
        {
            ...
            assertEquals("/clients/$ID_CLIENTE/entries/$ID_ENTIDAD", peticionRealizada.path)
    ...
    {
        private val entidadPatch = FondoDisponibleParaLaVentaDTO<Entrada>(false)
