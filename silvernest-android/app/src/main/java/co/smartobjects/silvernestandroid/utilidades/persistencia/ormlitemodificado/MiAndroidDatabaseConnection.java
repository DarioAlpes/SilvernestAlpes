...
public class MiAndroidDatabaseConnection implements DatabaseConnection
{
	private static final String ANDROID_VERSION = "VERSION__5.1-SNAPSHOT__";

	private static Logger logger = LoggerFactory.getLogger(com.j256.ormlite.android.AndroidDatabaseConnection.class);
	...
	public MiAndroidDatabaseConnection(SQLiteDatabase db, boolean readWrite)
	{
		this(db, readWrite, false);
	}

	public MiAndroidDatabaseConnection(SQLiteDatabase db, boolean readWrite, boolean cancelQueriesEnabled)
	{
		this.db = db;
	...
	{
		return MiAndroidCompiledStatement.execSql(db, statementStr, statementStr, NO_STRING_ARGS);
	}
	...
	public CompiledStatement compileStatement(String statement, StatementBuilder.StatementType type, FieldType[] argFieldTypes, int resultFlags, boolean cacheStore)
	{
		...
		CompiledStatement stmt = new MiAndroidCompiledStatement(statement, db, type, cancelQueriesEnabled, cacheStore);
	...
	{
		Cursor cursor = null;
		MiAndroidDatabaseResults results = null;
		try
		{
			cursor = db.rawQuery(statement, toStrings(args));
			results = new MiAndroidDatabaseResults(cursor, objectCache, true);
	...
	public long queryForLong(String statement, Object[] args, FieldType[] argFieldTypes) throws SQLException
	{
		...
		{
			cursor = db.rawQuery(statement, toStrings(args));
			results = new MiAndroidDatabaseResults(cursor, null, false);
			long result;
			if(results.first())
			{
