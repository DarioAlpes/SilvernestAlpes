...
@DisplayName("Recurso Categorias Sku")
internal class RecursoCategoriaSkuPruebas
{
    companion object
    {
    ...
    private fun darEntidadNegocioSegunIndice(indice: Int): CategoriaSku
    {
        return CategoriaSku(ID_CLIENTE,
                            indice.toLong(),
    ...
    private fun darEntidadDTOSegunIndice(indice: Int): CategoriaSkuDTO
    {
        return CategoriaSkuDTO(darEntidadNegocioSegunIndice(indice))
    }
    ...
    inner class Red
    {
        ...
        private lateinit var mockRecursoTodasEntidades: RecursoCategoriasSku
        private lateinit var mockRecursoEntidadEspecifica: RecursoCategoriasSku.RecursoCategoriaSku

        @[BeforeEach Throws(Exception::class)]
        fun antesDeCadaTest()
        {
            mockRecursoEntidadEspecifica = mockConDefaultAnswer(RecursoCategoriasSku.RecursoCategoriaSku::class.java)
            mockRecursoTodasEntidades = mockConDefaultAnswer(RecursoCategoriasSku::class.java)
            ...
            doReturn(mockRecursoTodasEntidades).`when`(mockRecursoCliente).darRecursoCategoriasSkus()
        ...
        inner class Crear
        {
            ...
            fun llama_la_funcion_crear_con_dto_correcto()
            {
                ...
                doReturn(CategoriaSkuDTO(entidadPruebas)).`when`(mockRecursoTodasEntidades).crear(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoCategoriasSku.RUTA}").request()
                    .post(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), CategoriaSkuDTO::class.java)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<CategoriaSkuDTO>()).`when`(mockRecursoTodasEntidades).darTodas()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoCategoriasSku.RUTA}").request().get(String::class.java)
        ...
        inner class ConsultarUna
        {
            ...
            {
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(CategoriaSkuDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).darPorId()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoCategoriasSku.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .get(CategoriaSkuDTO::class.java)
        ...
        inner class Actualizar
        {
            ...
            fun llama_la_funcion_actualizar_con_dto_correcto()
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(CategoriaSkuDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).actualizar(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoCategoriasSku.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .put(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), CategoriaSkuDTO::class.java)
        ...
        inner class Patch
        {
            ...
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPatch = FondoDisponibleParaLaVentaDTO<CategoriaSku>(dtoPruebas.disponibleParaLaVenta)
                doNothing().`when`(mockRecursoEntidadEspecifica).patch(entidadPatch)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoCategoriasSku.RUTA}/$ID_ENTIDAD_PRUEBAS")
                    .request()
    ...
    inner class Codigo
    {
        private val mockRepositorio = mockConDefaultAnswer(RepositorioCategoriasSkus::class.java)
        private var mockManejadorSeguridad = mockManejadorSeguridadUsuarioDeClienteSiempreTienePermisos
        private val recursoTodasEntidades: RecursoCategoriasSku by lazy { RecursoCategoriasSku(ID_CLIENTE, mockRepositorio, mockManejadorSeguridad) }
        private val recursoEntidadEspecifica: RecursoCategoriasSku.RecursoCategoriaSku by lazy { recursoTodasEntidades.darRecursosEntidadEspecifica(ID_ENTIDAD_PRUEBAS) }

        @Nested
        @DisplayName("Al crear")
        inner class Crear
        {
            private lateinit var entidadNegocio: CategoriaSku
            private lateinit var entidadDTO: CategoriaSkuDTO
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_entidad_referenciada_no_existe_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(CategoriaSkuDTO.CodigosError.ENTIDAD_REFERENCIADA_NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
            }
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun nombre_duplicado_cuando_el_repositorio_lanza_ErrorCreacionActualizacionPorDuplicidad()
                {
                    ...
                    doThrow(ErrorCreacionActualizacionPorDuplicidad("Categoria Sku"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(CategoriaSkuDTO.CodigosError.ENTIDAD_DUPLICADA_EN_BD, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    doThrow(ErrorDeCreacionActualizacionEntidad("Error creando CategoriaSku"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(CategoriaSkuDTO.CodigosError.ERROR_DE_BD_DESCONOCIDO, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                inner class PrecioPorDefectoInvalido
                {
                    ...
                    fun cuando_el_dto_tiene_precio_inferior_a_cero()
                    {
                        ...
                        kotlin.test.assertEquals(PrecioDTO.CodigosError.VALOR_INVALIDO, errorApi.codigoInterno)
                        verify(mockRepositorio, times(0)).crear(ID_CLIENTE, entidadNegocio)
                    }
                }
            }
            ...
            inner class Permisos
            {
                ...
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Categorias de Skus", PermisoBack.Accion.POST)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<CategoriaSku>())
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                doThrow(ErrorDeConsultaEntidad("Categoria Sku"))
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Categorias de Skus", PermisoBack.Accion.GET_TODOS)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class ConsultarUno
        {
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_correcto_cuando_el_repositorio_retorna_null()
            {
                ...
                assertEquals(CategoriaSkuDTO.CodigosError.NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).buscarPorId(ID_CLIENTE, ID_ENTIDAD_PRUEBAS)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Categorias de Skus", PermisoBack.Accion.GET_UNO)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Actualizar
        {
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                {
                    doThrow(ErrorDeJerarquiaPorCiclo("Categoria Sku", 1, 2))
                        .`when`(mockRepositorio)
                        .actualizar(ID_CLIENTE, entidadNegocio.id!!, entidadNegocio)

                    val errorApi = assertThrows<EntidadInvalida> { recursoEntidadEspecifica.actualizar(entidadDTO) }

                    assertEquals(CategoriaSkuDTO.CodigosError.CICLO_JERARQUIA, errorApi.codigoInterno)
                    verify(mockRepositorio).actualizar(ID_CLIENTE, entidadNegocio.id!!, entidadNegocio)
                }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Categorias de Skus", PermisoBack.Accion.PUT)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Patch
        {
            ...
            {
                val entidadPatch = FondoDisponibleParaLaVentaDTO<CategoriaSku>(entidadDTO.disponibleParaLaVenta)
                doNothing()
                    .`when`(mockRepositorio)
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    kotlin.test.assertEquals(CategoriaSkuDTO.CodigosError.ERROR_DE_BD_DESCONOCIDO, errorApi.codigoInterno)
                    verify(mockRepositorio)
                        .actualizarCamposIndividuales(
                                ID_CLIENTE,
                                entidadDTO.id!!,
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Categorias de Skus", PermisoBack.Accion.PATCH)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Eliminar
        {
            ...
            fun lanza_excepcion_con_codigo_interno_entidad_referenciada_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                kotlin.test.assertEquals(CategoriaSkuDTO.CodigosError.ENTIDAD_REFERENCIADA, errorApi.codigoInterno)
                verify(mockRepositorio).eliminarPorId(ID_CLIENTE, entidadNegocio.id!!)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Categorias de Skus", PermisoBack.Accion.DELETE)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
