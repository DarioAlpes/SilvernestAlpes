...
@DisplayName("Recurso Skus")
internal class RecursoSkusPruebas
{
    companion object
    {
    ...
    inner class Red
    {
        ...
        private lateinit var mockRecursoTodasEntidades: RecursoSkus
        private lateinit var mockRecursoEntidadEspecifica: RecursoSkus.RecursoSku

        @[BeforeEach Throws(Exception::class)]
        fun antesDeCadaTest()
        {
            mockRecursoEntidadEspecifica = mockConDefaultAnswer(RecursoSkus.RecursoSku::class.java)
            mockRecursoTodasEntidades = mockConDefaultAnswer(RecursoSkus::class.java)
            ...
            doReturn(mockRecursoTodasEntidades).`when`(mockRecursoCliente).darRecursoSkus()
        ...
        inner class Crear
        {
            ...
            fun llama_la_funcion_crear_con_dto_correcto()
            {
                ...
                doReturn(SkuDTO(entidadPruebas)).`when`(mockRecursoTodasEntidades).crear(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoSkus.RUTA}").request()
                    .post(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), SkuDTO::class.java)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<SkuDTO>()).`when`(mockRecursoTodasEntidades).darTodas()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoSkus.RUTA}").request().get(String::class.java)
        ...
        inner class ConsultarUna
        {
            ...
            {
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(SkuDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).darPorId()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoSkus.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .get(SkuDTO::class.java)
        ...
        inner class Actualizar
        {
            ...
            fun llama_la_funcion_actualizar_con_dto_correcto()
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(SkuDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).actualizar(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoSkus.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .put(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), SkuDTO::class.java)
        ...
        inner class Patch
        {
            ...
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPatch = FondoDisponibleParaLaVentaDTO<Sku>(dtoPruebas.disponibleParaLaVenta)
                doNothing().`when`(mockRecursoEntidadEspecifica).patch(entidadPatch)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoSkus.RUTA}/$ID_ENTIDAD_PRUEBAS")
                    .request()
    ...
    inner class Codigo
    {
        private val mockRepositorio = mockConDefaultAnswer(RepositorioSkus::class.java)
        private var mockManejadorSeguridad = mockManejadorSeguridadUsuarioDeClienteSiempreTienePermisos
        private val recursoTodasEntidades: RecursoSkus by lazy { RecursoSkus(ID_CLIENTE, mockRepositorio, mockManejadorSeguridad) }
        private val recursoEntidadEspecifica: RecursoSkus.RecursoSku by lazy { recursoTodasEntidades.darRecursosEntidadEspecifica(ID_ENTIDAD_PRUEBAS) }

        @Nested
        @DisplayName("Al crear")
        inner class Crear
        {
            private lateinit var entidadNegocio: Sku
            private lateinit var entidadDTO: SkuDTO
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_entidad_referenciada_no_existe_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(SkuDTO.CodigosError.ENTIDAD_REFERENCIADA_NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
            }
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun nombre_duplicado_cuando_el_repositorio_lanza_ErrorCreacionActualizacionPorDuplicidad()
                {
                    ...
                    doThrow(ErrorCreacionActualizacionPorDuplicidad("Sku"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(SkuDTO.CodigosError.ENTIDAD_DUPLICADA_EN_BD, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    assertEquals(SkuDTO.CodigosError.ERROR_DE_BD_DESCONOCIDO, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
            ...
            inner class Permisos
            {
                ...
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Skus", PermisoBack.Accion.POST)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doThrow(ErrorDeConsultaEntidad("Sku"))
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Skus", PermisoBack.Accion.GET_TODOS)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class ConsultarUno
        {
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_correcto_cuando_el_repositorio_retorna_null()
            {
                ...
                assertEquals(SkuDTO.CodigosError.NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).buscarPorId(ID_CLIENTE, ID_ENTIDAD_PRUEBAS)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Skus", PermisoBack.Accion.GET_UNO)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Actualizar
        {
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Skus", PermisoBack.Accion.PUT)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Patch
        {
            ...
            {
                val entidadPatch = FondoDisponibleParaLaVentaDTO<Sku>(entidadDTO.disponibleParaLaVenta)
                doNothing()
                    .`when`(mockRepositorio)
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    kotlin.test.assertEquals(SkuDTO.CodigosError.ERROR_DE_BD_DESCONOCIDO, errorApi.codigoInterno)
                    verify(mockRepositorio)
                        .actualizarCamposIndividuales(
                                ID_CLIENTE,
                                entidadDTO.id!!,
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Skus", PermisoBack.Accion.PATCH)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Eliminar
        {
            ...
            fun lanza_excepcion_con_codigo_interno_entidad_referenciada_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(SkuDTO.CodigosError.ENTIDAD_REFERENCIADA, errorApi.codigoInterno)
                verify(mockRepositorio).eliminarPorId(ID_CLIENTE, entidadNegocio.id!!)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "Skus", PermisoBack.Accion.DELETE)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
