...
@DisplayName("Recurso libros de precios")
internal class RecursoLibrosDePreciosPruebas
{
    companion object
    {
        ...
        internal fun darEntidadNegocioSegunIndice(indice: Int): LibroDePrecios
        {
            return LibroDePrecios(
                    ...
                    List(indice + 1) {
                        PrecioEnLibro(
                                Precio(Decimal(1000) * indice * it, indice.toLong() + 10 * it),
                                indice.toLong() + 10 * it
                                     )
                    }.toSet()
                                 )
        ...
        internal fun darEntidadDTOSegunIndice(indice: Int): LibroDePreciosDTO
        {
            return LibroDePreciosDTO(darEntidadNegocioSegunIndice(indice))
        }
    }
    ...
    inner class Red
    {
        ...
        private lateinit var mockRecursoTodasEntidades: RecursoLibrosDePrecios
        private lateinit var mockRecursoEntidadEspecifica: RecursoLibrosDePrecios.RecursoLibroDePrecios

        @[BeforeEach Throws(Exception::class)]
        fun antesDeCadaTest()
        {
            mockRecursoEntidadEspecifica = mockConDefaultAnswer(RecursoLibrosDePrecios.RecursoLibroDePrecios::class.java)
            mockRecursoTodasEntidades = mockConDefaultAnswer(RecursoLibrosDePrecios::class.java)
            ...
            doReturn(mockRecursoTodasEntidades).`when`(mockRecursoCliente).darRecursoLibrosDePrecios()
        ...
        inner class Crear
        {
            ...
            fun llama_la_funcion_crear_con_dto_correcto()
            {
                ...
                doReturn(LibroDePreciosDTO(entidadPruebas)).`when`(mockRecursoTodasEntidades).crear(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDePrecios.RUTA}").request()
                    .post(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), LibroDePreciosDTO::class.java)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<LibroDePreciosDTO>()).`when`(mockRecursoTodasEntidades).darTodas()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDePrecios.RUTA}").request().get(String::class.java)
        ...
        inner class ConsultarUna
        {
            ...
            {
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(LibroDePreciosDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).darPorId()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDePrecios.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .get(LibroDePreciosDTO::class.java)
        ...
        inner class Actualizar
        {
            ...
            fun llama_la_funcion_actualizar_con_dto_correcto()
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(LibroDePreciosDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).actualizar(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDePrecios.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .put(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), LibroDePreciosDTO::class.java)
    ...
    inner class Codigo
    {
        private val mockRepositorio = mockConDefaultAnswer(RepositorioLibrosDePrecios::class.java)
        private var mockManejadorSeguridad = mockManejadorSeguridadUsuarioDeClienteSiempreTienePermisos
        private val recursoTodasEntidades: RecursoLibrosDePrecios by lazy { RecursoLibrosDePrecios(ID_CLIENTE, mockRepositorio, mockManejadorSeguridad) }
        private val recursoEntidadEspecifica: RecursoLibrosDePrecios.RecursoLibroDePrecios by lazy { recursoTodasEntidades.darRecursosEntidadEspecifica(ID_ENTIDAD_PRUEBAS) }

        @Nested
        @DisplayName("Al crear")
        inner class Crear
        {
            private lateinit var entidadNegocio: LibroDePrecios
            private lateinit var entidadDTO: LibroDePreciosDTO
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_entidad_referenciada_no_existe_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(LibroDePreciosDTO.CodigosError.ENTIDAD_REFERENCIADA_NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
            }
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun nombre_duplicado_cuando_el_repositorio_lanza_ErrorCreacionActualizacionPorDuplicidad()
                {
                    ...
                    doThrow(ErrorCreacionActualizacionPorDuplicidad("LibroDePrecios"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(LibroDePreciosDTO.CodigosError.ENTIDAD_DUPLICADA_EN_BD, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    doThrow(ErrorDeCreacionActualizacionEntidad("Error creando LibroDePrecios"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(LibroDePreciosDTO.CodigosError.ERROR_DE_BD_DESCONOCIDO, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                inner class PreciosInvalidos
                {
                    @Test
                    fun cuando_el_dto_trae_precios_vacios()
                    {
                        val errorApi = assertThrows<EntidadInvalida> {
                            recursoTodasEntidades.crear(entidadDTO.copy(precios = listOf()))
                        }

                        assertEquals(LibroDePreciosDTO.CodigosError.PRECIOS_VACIOS, errorApi.codigoInterno)
                        verify(mockRepositorio, times(0)).crear(ID_CLIENTE, entidadNegocio)
                    }
                }
            }
            ...
            inner class Permisos
            {
                ...
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDePrecios", PermisoBack.Accion.POST)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<LibroDePrecios>())
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                doThrow(ErrorDeConsultaEntidad("LibroDePrecios"))
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDePrecios", PermisoBack.Accion.GET_TODOS)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class ConsultarUno
        {
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_correcto_cuando_el_repositorio_retorna_null()
            {
                ...
                assertEquals(LibroDePreciosDTO.CodigosError.NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).buscarPorId(ID_CLIENTE, ID_ENTIDAD_PRUEBAS)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDePrecios", PermisoBack.Accion.GET_UNO)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Actualizar
        {
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                inner class PreciosInvalidos
                {
                    ...
                    {
                        val errorApi = assertThrows<EntidadInvalida> {
                            recursoEntidadEspecifica.actualizar(entidadDTO.copy(precios = listOf()))
                        }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDePrecios", PermisoBack.Accion.PUT)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Eliminar
        {
            ...
            fun lanza_excepcion_con_codigo_interno_entidad_referenciada_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(LibroDePreciosDTO.CodigosError.ENTIDAD_REFERENCIADA, errorApi.codigoInterno)
                verify(mockRepositorio).eliminarPorId(ID_CLIENTE, entidadNegocio.id!!)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDePrecios", PermisoBack.Accion.DELETE)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
