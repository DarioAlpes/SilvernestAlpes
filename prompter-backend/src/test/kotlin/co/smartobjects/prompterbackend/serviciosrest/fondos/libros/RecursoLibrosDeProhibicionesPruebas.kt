...
@DisplayName("Recurso libros de prohibiciones")
internal class RecursoLibrosDeProhibicionesPruebas
{
    companion object
    {
    ...
    private fun darEntidadNegocioSegunIndice(indice: Int): LibroDeProhibiciones
    {
        return LibroDeProhibiciones(
                ...
                List(indice + 1) {
                    Prohibicion.DeFondo(it + 10L)
                }.toSet(),
                List(indice + 1) {
                    Prohibicion.DePaquete(it + 10L)
                }.toSet()
                                   )
    }

    private fun darEntidadDTOSegunIndice(indice: Int): LibroDeProhibicionesDTO
    {
        return LibroDeProhibicionesDTO(darEntidadNegocioSegunIndice(indice))
    }
    ...
    inner class Red
    {
        ...
        private lateinit var mockRecursoTodasEntidades: RecursoLibrosDeProhibiciones
        private lateinit var mockRecursoEntidadEspecifica: RecursoLibrosDeProhibiciones.RecursoLibroDeProhibiciones

        @[BeforeEach Throws(Exception::class)]
        fun antesDeCadaTest()
        {
            mockRecursoEntidadEspecifica = mockConDefaultAnswer(RecursoLibrosDeProhibiciones.RecursoLibroDeProhibiciones::class.java)
            mockRecursoTodasEntidades = mockConDefaultAnswer(RecursoLibrosDeProhibiciones::class.java)
            ...
            doReturn(mockRecursoTodasEntidades).`when`(mockRecursoCliente).darRecursoLibrosDeProhibiciones()
        ...
        inner class Crear
        {
            ...
            fun llama_la_funcion_crear_con_dto_correcto()
            {
                ...
                doReturn(LibroDeProhibicionesDTO(entidadPruebas)).`when`(mockRecursoTodasEntidades).crear(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDeProhibiciones.RUTA}").request()
                    .post(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), LibroDeProhibicionesDTO::class.java)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<LibroDeProhibicionesDTO>()).`when`(mockRecursoTodasEntidades).darTodas()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDeProhibiciones.RUTA}").request().get(String::class.java)
        ...
        inner class ConsultarUna
        {
            ...
            {
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(LibroDeProhibicionesDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).darPorId()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDeProhibiciones.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .get(LibroDeProhibicionesDTO::class.java)
        ...
        inner class Actualizar
        {
            ...
            fun llama_la_funcion_actualizar_con_dto_correcto()
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(LibroDeProhibicionesDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).actualizar(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosDeProhibiciones.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .put(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), LibroDeProhibicionesDTO::class.java)
    ...
    inner class Codigo
    {
        private val mockRepositorio = mockConDefaultAnswer(RepositorioLibrosDeProhibiciones::class.java)
        private var mockManejadorSeguridad = mockManejadorSeguridadUsuarioDeClienteSiempreTienePermisos
        private val recursoTodasEntidades: RecursoLibrosDeProhibiciones by lazy { RecursoLibrosDeProhibiciones(ID_CLIENTE, mockRepositorio, mockManejadorSeguridad) }
        private val recursoEntidadEspecifica: RecursoLibrosDeProhibiciones.RecursoLibroDeProhibiciones by lazy { recursoTodasEntidades.darRecursosEntidadEspecifica(ID_ENTIDAD_PRUEBAS) }

        @Nested
        @DisplayName("Al crear")
        inner class Crear
        {
            private lateinit var entidadNegocio: LibroDeProhibiciones
            private lateinit var entidadDTO: LibroDeProhibicionesDTO
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_entidad_referenciada_no_existe_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(LibroDeProhibicionesDTO.CodigosError.ENTIDAD_REFERENCIADA_NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
            }
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun nombre_duplicado_cuando_el_repositorio_lanza_ErrorCreacionActualizacionPorDuplicidad()
                {
                    ...
                    doThrow(ErrorCreacionActualizacionPorDuplicidad("LibroDeProhibiciones"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(LibroDeProhibicionesDTO.CodigosError.ENTIDAD_DUPLICADA_EN_BD, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    doThrow(ErrorDeCreacionActualizacionEntidad("Error creando LibroDeProhibiciones"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(LibroDeProhibicionesDTO.CodigosError.ERROR_DE_BD_DESCONOCIDO, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                inner class NombreInvalido
                {
                    ...
                    fun cuando_el_dto_tiene_nombre_vacio()
                    {
                        ...
                        assertEquals(LibroDTO.CodigosError.NOMBRE_INVALIDO, errorApi.codigoInterno)
                ...
                @Nested
                @DisplayName("Prohibiciones invalidas")
                inner class ProhibicionesInvalidas
                {
                    @Test
                    fun cuando_el_dto_trae_precios_vacios()
                    {
                        val errorApi = assertThrows<EntidadInvalida> {
                            recursoTodasEntidades.crear(
                                    entidadDTO.copy(prohibicionesDeFondo = listOf(), prohibicionesDePaquete = listOf())
                                                       )
                        }

                        assertEquals(LibroDeProhibicionesDTO.CodigosError.NO_HAY_PROHIBICIONES_DE_NINGUN_TIPO, errorApi.codigoInterno)
                        verify(mockRepositorio, times(0)).crear(ID_CLIENTE, entidadNegocio)
                    }
                }
            }
            ...
            inner class Permisos
            {
                ...
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDeProhibiciones", PermisoBack.Accion.POST)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<LibroDeProhibiciones>())
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                doThrow(ErrorDeConsultaEntidad("LibroDeProhibiciones"))
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDeProhibiciones", PermisoBack.Accion.GET_TODOS)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class ConsultarUno
        {
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_correcto_cuando_el_repositorio_retorna_null()
            {
                ...
                assertEquals(LibroDeProhibicionesDTO.CodigosError.NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).buscarPorId(ID_CLIENTE, ID_ENTIDAD_PRUEBAS)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDeProhibiciones", PermisoBack.Accion.GET_UNO)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Actualizar
        {
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                inner class ProhibicionesInvalidas
                {
                    ...
                    {
                        val errorApi = assertThrows<EntidadInvalida> {
                            recursoEntidadEspecifica.actualizar(
                                    entidadDTO.copy(prohibicionesDeFondo = listOf(), prohibicionesDePaquete = listOf())
                                                               )
                        }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDeProhibiciones", PermisoBack.Accion.PUT)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Eliminar
        {
            ...
            fun lanza_excepcion_con_codigo_interno_entidad_referenciada_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(LibroDeProhibicionesDTO.CodigosError.ENTIDAD_REFERENCIADA, errorApi.codigoInterno)
                verify(mockRepositorio).eliminarPorId(ID_CLIENTE, entidadNegocio.id!!)
            }
            ...
            {
                doThrow(co.smartobjects.persistencia.excepciones.ErrorEliminandoEntidad(Random().nextLong(), "Nombre entidad"))
                    .`when`(mockRepositorio)
                    .eliminarPorId(ID_CLIENTE, entidadNegocio.id!!)
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroDeProhibiciones", PermisoBack.Accion.DELETE)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
