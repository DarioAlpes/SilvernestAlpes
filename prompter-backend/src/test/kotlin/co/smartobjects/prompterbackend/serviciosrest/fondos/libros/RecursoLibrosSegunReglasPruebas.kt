...
@DisplayName("Recurso libro según reglas")
internal class RecursoLibrosSegunReglasPruebas
{
    companion object
    {
    ...
    private fun darEntidadNegocioSegunIndice(indice: Int): LibroSegunReglas
    {
    ...
    private fun darEntidadDTOSegunIndice(indice: Int): LibroSegunReglasDTO
    {
        return LibroSegunReglasDTO(darEntidadNegocioSegunIndice(indice))
    }
    ...
    inner class Red
    {
        ...
        private lateinit var mockRecursoTodasEntidades: RecursoLibrosSegunReglas
        private lateinit var mockRecursoEntidadEspecifica: RecursoLibrosSegunReglas.RecursoLibroSegunReglas

        @[BeforeEach Throws(Exception::class)]
        fun antesDeCadaTest()
        {
            mockRecursoEntidadEspecifica = mockConDefaultAnswer(RecursoLibrosSegunReglas.RecursoLibroSegunReglas::class.java)
            mockRecursoTodasEntidades = mockConDefaultAnswer(RecursoLibrosSegunReglas::class.java)
            ...
            doReturn(mockRecursoTodasEntidades).`when`(mockRecursoCliente).darRecursoLibrosSegunReglas()
        ...
        inner class Crear
        {
            ...
            fun llama_la_funcion_crear_con_dto_correcto()
            {
                ...
                doReturn(LibroSegunReglasDTO(entidadPruebas)).`when`(mockRecursoTodasEntidades).crear(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosSegunReglas.RUTA}").request()
                    .post(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), LibroSegunReglasDTO::class.java)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<LibroSegunReglasDTO>()).`when`(mockRecursoTodasEntidades).darTodas()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosSegunReglas.RUTA}").request().get(String::class.java)
        ...
        inner class ConsultarUna
        {
            ...
            {
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(LibroSegunReglasDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).darPorId()

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosSegunReglas.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .get(LibroSegunReglasDTO::class.java)
        ...
        inner class Actualizar
        {
            ...
            fun llama_la_funcion_actualizar_con_dto_correcto()
            {
                val dtoPruebas = darEntidadDTOSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                val entidadPruebas = darEntidadNegocioSegunIndice(ID_ENTIDAD_PRUEBAS.toInt())
                doReturn(LibroSegunReglasDTO(entidadPruebas)).`when`(mockRecursoEntidadEspecifica).actualizar(dtoPruebas)

                target.path("${RecursoClientes.RUTA}/$ID_CLIENTE/${RecursoLibrosSegunReglas.RUTA}/$ID_ENTIDAD_PRUEBAS").request()
                    .put(Entity.entity(dtoPruebas, MediaType.APPLICATION_JSON_TYPE), LibroSegunReglasDTO::class.java)
    ...
    inner class Codigo
    {
        private val mockRepositorio = mockConDefaultAnswer(RepositorioLibrosSegunReglas::class.java)
        private var mockManejadorSeguridad = mockManejadorSeguridadUsuarioDeClienteSiempreTienePermisos
        private val recursoTodasEntidades: RecursoLibrosSegunReglas by lazy { RecursoLibrosSegunReglas(ID_CLIENTE, mockRepositorio, mockManejadorSeguridad) }
        private val recursoEntidadEspecifica: RecursoLibrosSegunReglas.RecursoLibroSegunReglas by lazy { recursoTodasEntidades.darRecursosEntidadEspecifica(ID_ENTIDAD_PRUEBAS) }

        @Nested
        @DisplayName("Al crear")
        inner class Crear
        {
            private lateinit var entidadNegocio: LibroSegunReglas
            private lateinit var entidadDTO: LibroSegunReglasDTO
            ...
            fun lanza_excepcion_EntidadNoExiste_con_codigo_interno_entidad_referenciada_no_existe_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(LibroSegunReglasDTO.CodigosError.ENTIDAD_REFERENCIADA_NO_EXISTE, errorApi.codigoInterno)
                verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
            }
            ...
            inner class LanzanExcepcionEntidadInvalida
            {
                ...
                fun nombre_duplicado_cuando_el_repositorio_lanza_ErrorCreacionActualizacionPorDuplicidad()
                {
                    ...
                    doThrow(ErrorCreacionActualizacionPorDuplicidad("LibroSegunReglas"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)

                    val errorApi = assertThrows<EntidadInvalida> { recursoTodasEntidades.crear(entidadDTO) }

                    assertEquals(LibroSegunReglasDTO.CodigosError.ENTIDAD_DUPLICADA_EN_BD, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                fun desconocido_cuando_el_repositorio_lanza_ErrorDeCreacionActualizacionEntidad()
                {
                    ...
                    doThrow(ErrorDeCreacionActualizacionEntidad("Error creando LibroSegunReglas"))
                        .`when`(mockRepositorio)
                        .crear(ID_CLIENTE, entidadConIdNulo)
                ...
                @Test
                fun el_libro_asociado_repite_reglas_en_otro_libro_segun_reglas_el_repositorio_lanza_ErrorCreacionViolacionDeRestriccion()
                {
                    val entidadConIdNulo = entidadNegocio.copiar(id = null)
                    ...
                    assertEquals(LibroSegunReglasDTO.CodigosError.EL_LIBRO_ASOCIADO_REPITE_REGLAS, errorApi.codigoInterno)
                    verify(mockRepositorio).crear(ID_CLIENTE, entidadConIdNulo)
                }
                ...
                inner class NombreInvalido
                {
                    ...
                    fun cuando_el_dto_tiene_nombre_vacio()
                    {
                        ...
                        assertEquals(LibroSegunReglasDTO.CodigosError.NOMBRE_INVALIDO, errorApi.codigoInterno)
            ...
            inner class Permisos
            {
                ...
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroSegunReglas", PermisoBack.Accion.POST)
        ...
        inner class ConsultarTodos
        {
            ...
            {
                doReturn(sequenceOf<LibroSegunReglas>())
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                doThrow(ErrorDeConsultaEntidad("LibroSegunReglas"))
                    .`when`(mockRepositorio)
                    .listar(ID_CLIENTE)
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroSegunReglas", PermisoBack.Accion.GET_TODOS)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class ConsultarUno
        {
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroSegunReglas", PermisoBack.Accion.GET_UNO)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Actualizar
        {
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroSegunReglas", PermisoBack.Accion.PUT)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
        ...
        inner class Eliminar
        {
            ...
            fun lanza_excepcion_con_codigo_interno_entidad_referenciada_cuando_el_repositorio_lanza_ErrorDeLlaveForanea()
            {
                ...
                assertEquals(LibroSegunReglasDTO.CodigosError.ENTIDAD_REFERENCIADA, errorApi.codigoInterno)
                verify(mockRepositorio).eliminarPorId(ID_CLIENTE, entidadNegocio.id!!)
            }
            ...
            {
                // Se usa el nombre del permiso directamente en lugar de usar la constante para garantizar que falle si es cambiado (Este valor se escribe en BD)
                private val permisoBuscadoNegocio = PermisoBack(ID_CLIENTE, "LibroSegunReglas", PermisoBack.Accion.DELETE)
                private val permisoBuscadoDTO = PermisoBackDTO(permisoBuscadoNegocio)
