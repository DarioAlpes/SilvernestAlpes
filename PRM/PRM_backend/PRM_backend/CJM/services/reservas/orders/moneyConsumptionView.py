...
PERSON_MONEY_CONSUMPTIONS_VIEW_NAME = "person-money-consumptions"
app = Blueprint(PERSON_MONEY_CONSUMPTIONS_VIEW_NAME, __name__)


@app.route('/clients/<int:id_client>/person-money-consumptions/',
           methods=['GET'], strict_slashes=False)
@with_json_bodyless
def list_amount_consumptions(id_client):
    """
    Lista los consumos por dinero existentes.
    ...
    def list_amount_consumptions_on_namespace(id_current_client):
        return PersonConsumptionByMoney.list()
    return on_client_namespace(id_client,
                               list_amount_consumptions_on_namespace,
                               action=Role.READ_ACTION,
                               view=PERSON_MONEY_CONSUMPTIONS_VIEW_NAME)


@app.route('/clients/<int:id_client>/reservations/<int:id_reservation>/'
           'persons-reservations/<int:id_person_reservation>/person-money-consumptions/',
           methods=['GET'], strict_slashes=False)
@with_json_bodyless
def list_money_consumptions_by_person_reservation(id_client, id_reservation, id_person_reservation):
    """
    Lista los consumos por dinero existentes para la reserva de persona dada.
    :param id_client: id del cliente asociado
    ...
    def list_money_consumptions_by_person_reservation_on_namespace(id_current_client):
        reservation, person_reservation = get_reservation_and_person_reservation_on_namespace_with_permission(id_current_client,
                                                                                                              ...
                                                                                                              Role.READ_ACTION,
                                                                                                              PERSON_MONEY_CONSUMPTIONS_VIEW_NAME)
        return PersonConsumptionByMoney.list_by_ids_person_reservation(person_reservation.idReserva,
                                                                       person_reservation.key.id())

    return on_client_namespace(id_client,
                               list_money_consumptions_by_person_reservation_on_namespace,
                               secured=False)
